...
- **多模型 Provider** — 原生支持 Ollama / OpenAI-compatible / Anthropic / DeepSeek / Gemini / Azure OpenAI / SiliconFlow / Groq / Together / Mistral / OpenRouter / Qwen / Doubao / Hunyuan / Perplexity / xAI / Cohere 等开箱即用
...
- **Providers**：原生接入支持 17 种 LLM 提供商（Ollama, OpenAI-compatible, Anthropic, DeepSeek, Gemini, Azure, SiliconFlow, Groq, Mistral, Cohere 等）
...
- **LLM / AI 安全（🌟 亮点）**
  - [x] OWASP LLM Top 10 攻击包（10 类攻击，中英双语，含潜伏式多轮越狱等）
  - [x] RAG 上下文污染攻击包（18 用例 / 5 大类：文档注入、检索操纵、嵌入混淆、来源伪造、RAG 投毒）
...
- **Web 控制台 (GUI)**
  - [x] 提供极客红队科技风的可视化仪表盘 (Dashboard)
  - [x] 实时渲染系统级性能指标与活跃扫描任务
  - [x] 提供大模型 API 配置表单与扫描向导
- **多模型 Provider**
  - [x] 支持本地 Ollama 接口连通性测试与生成
  - [x] 支持 OpenAI 兼容接口（含硅基流动、Groq、Mistral、Together 等 10+ 第三方平台）
  - [x] 支持 Anthropic (Claude) Messages API 原生接口
  - [x] 支持 DeepSeek API 原生接口
  - [x] Gemini API 与 Azure OpenAI 原生支持
  - [x] Cohere 原生接口支持
- **Web 渗透测试**
...
- **进阶 Web 渗透**
  ...
  - [x] 认证绕过 / 权限提升自动化检测（IDOR / 越权）

- **MCP 服务化**
  - [x] 原生封装为标准的 Model Context Protocol (MCP) Server，可通过 `nullbunny mcp start` 启动，并被其他 AI Agent 调用执行自动化扫描。
- **基础设施安全**
  - [x] TCP 端口扫描与服务连通性探测
  - [x] 服务识别与 Banner 抓取
  - [x] 子域名枚举与字典解析
